...
extends ModelBox {
    ...
    private final boolean[] visibleQuads = new boolean[]{true, true, true, true, true, true};
    ...
    public void render(Tessellator p_78245_1_, float p_78245_2_) {
        for (int i = 0; i < this.quads.length; ++i) {
            if (!this.visibleQuads[i]) continue;
            this.quads[i].draw(p_78245_1_, p_78245_2_);
        }
    }

    public void hideQuad(int quad) {
        if (quad >= 0 && quad < this.visibleQuads.length) {
            this.visibleQuads[quad] = false;
        }
    }

    /** Moves the minimum-Y ring inward without changing its UV coordinates. */
    public void insetMinY(float amount) {
        this.vertices[0].vector3D.yCoord += amount;
        this.vertices[1].vector3D.yCoord += amount;
        this.vertices[4].vector3D.yCoord += amount;
        this.vertices[5].vector3D.yCoord += amount;
    }

    /** Separates an upper joint segment from its lower segment without changing UVs. */
    public void expandXZ(float amount) {
        double centerX = (this.vertices[0].vector3D.xCoord + this.vertices[1].vector3D.xCoord) * 0.5;
        double centerZ = (this.vertices[0].vector3D.zCoord + this.vertices[4].vector3D.zCoord) * 0.5;
        for (PositionTextureVertex vertex : this.vertices) {
            vertex.vector3D.xCoord += vertex.vector3D.xCoord < centerX ? -amount : amount;
            vertex.vector3D.zCoord += vertex.vector3D.zCoord < centerZ ? -amount : amount;
        }
    }
